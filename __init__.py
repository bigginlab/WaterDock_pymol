...
def __init__(self):
    ...
    self.menuBar.addmenuitem('Plugin', 'command', 'Apo-WaterDock',
                             label='Apo-WaterDock', command=lambda s=self: option1())
    self.menuBar.addmenuitem('Plugin', 'command', 'Holo-Waterdock',
                             label='Holo-waterdock', command=lambda s=self: option2())
...
class inputdata1:
    def __init__(self):
        ...
        Label(self.window, text="Import Protein from File (pdbqt)").grid(
            row=2, column=0, sticky=E)
        self.E1 = Entry(self.window, textvariable=self.profilenameentry)
        self.E1.grid(row=2, column=1)
        Button(self.window, text="Choose", command=self.profilechoose).grid(
            row=2, column=2, sticky=W)

        Label(self.window, text="Identifying the binding site").grid(
            row=4, column=0, sticky=W)

        Radiobutton(self.window, text="Enter Center of Box (A)",
                    variable=self.ligopt, value='1').grid(row=5, column=0, sticky=E)
        ...
        Radiobutton(self.window, text="Co-ordinates from Ligand file (mol2/pdb/pdbqt)",
                    variable=self.ligopt, value='2').grid(row=6, column=0, sticky=E)
        self.E2 = Entry(self.window, textvariable=self.ligfilename)
        self.E2.grid(row=6, column=1)
        Button(self.window, text="Choose", command=self.ligfilechoose).grid(
            row=6, column=2, sticky=W)

        Button(self.window, text="Run", command=self.rungui).grid(
            row=13, column=3, sticky=E)
        Button(self.window, text="Cancel", command=self.byebye).grid(
            row=13, column=4, sticky=E)
    ...
    def ligfilechoose(self):
        self.ligfilename = tkinter.filedialog.askopenfilename(filetypes=(
            ("pdb files", "*.pdb"), ("pdbqt files", "*.pdbqt"), ("mol2 files", "*.mol2")))
    ...
    def rungui(self):
        ...
        if int(os.path.isfile(proteinfile)) == 0:
            tkinter.messagebox.showerror(
                title='Missing File', message='Protein File does not exist')
            sys.exit()
        ...
        if ligandoption == '2':
            if hasattr(self.E2, 'get'):
                ...
                if int(os.path.isfile(ligandfile)) == 0:
                    tkinter.messagebox.showerror(
                        title='Missing File', message='Ligand File does not exist')
                    sys.exit()
...
class inputdata2:
    def __init__(self):
        ...
        Label(self.window, text="Import Ligand from File (pdb/mol2)").grid(row=3,
                                                                           column=0, sticky=E)
        self.E2 = Entry(self.window, textvariable=self.ligfilename)
...
class vinapath:
    def __init__(self):
        ...
        Label(self.window, text="Enter the path to the vina executable").grid(
            row=2, column=0, sticky=E)
...
#	Use dialog to force selection of something an restrict false entries

        Button(self.window, text="Choose", command=self.vinafilechoose).grid(
            row=2, column=2, sticky=W)

        Button(self.window, text="OK", command=self.okay).grid(
            row=4, column=3, sticky=E)
        Button(self.window, text="Cancel", command=self.byebye).grid(
            row=4, column=4, sticky=E)
