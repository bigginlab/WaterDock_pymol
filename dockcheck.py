...
def main(proteinfilename, ligandfilename, vinacomd):
    ...
    for i in range(0, numtrialwaters):
        ...
        if watprodist[i] < 3.6 and watprodist[i] > 2.00:
            ...
            comd = vinacomd + ' --receptor ' + proteinfilename + ' \
                --num_modes 1 --exhaustiveness 5 --ligand water.pdbqt \
                    --size_x 0.5 --size_y 0.5 --size_z 0.5 --out waterout.pdbqt \
                        --center_x ' + str(trialwatercoods[i, 0]) + ' --center_y ' \
                            + str(trialwatercoods[i, 1]) + ' --center_z ' + str(trialwatercoods[i, 2])
            os.system(comd)
